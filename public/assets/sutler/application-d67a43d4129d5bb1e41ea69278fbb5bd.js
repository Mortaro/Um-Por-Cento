...
(function() {
  $(function() {
    autosize($('textarea'));
    $('.field_with_errors input').change(function() {
      return $(this).closest('.form-group').find('.field_with_errors').removeClass('field_with_errors');
    });
    return $('.progress-upload').each(function() {
      var that;
      that = $(this);
      return that.find('[type=file]').fileupload({
        dataType: 'json',
        progressall: function(e, data) {
          var progress;
          that.find('.btn').fadeOut('slow');
          progress = parseInt(data.loaded / data.total * 100, 10);
          return that.find('.progress-bar').css('width', progress + '%');
        },
        done: function(e, data) {
          var value;
          that.find('.btn').fadeIn('slow', function() {
            return that.find('.progress-bar').css('width', '0%');
          });
          ...
          value = [];
          that.closest('.form-group').find('img').each(function() {
            return value.push($(this).attr('src'));
          });
          return that.find('input[type=hidden]').val(value.join(','));
        }
      });
    });
  });
...
(function() {
  $(function() {
    $(document).on('click', '.thumbnail-wrapper a', function() {
      var group, value;
    ...
    return $('.progress-upload').each(function() {
      ...
      return that.find('[type=file]').fileupload({
        ...
        progressall: function(e, data) {
          ...
          var image, trash, url, value, wrapper;
